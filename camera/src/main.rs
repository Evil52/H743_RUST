...
use display::{FRAME_H, FRAME_W, FRAME_WORDS};
use ov2640::Ov2640;
...
async fn main(spawner: Spawner) {
    ...
    // rgb=false (BGR subpixel order) + inversion match this WeAct panel.
    let mut lcd = st7735_lcd::ST7735::new(spi_dev, dc, rst, false, true, 160, 128);
    unwrap!(lcd.init(&mut Delay).map_err(|_| ()));
    unwrap!(lcd
